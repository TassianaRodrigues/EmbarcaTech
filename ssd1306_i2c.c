/*CONTÉM DRIVER SSD1306 E CÓDIGO DA ATIVIDADE
OS TRECHOS FORAM COMENTADOS PARA IDENTIFICAR A PARTE CORRESPONDENTE A BIBLIOTECA 
E AO CÓDIGO DA ATIVIDADE*/
/**
...
//Início das configurações do código do semáforo
#define LED_R_PIN 13
#define LED_G_PIN 11
#define LED_B_PIN 12
#define BTN_A_PIN 5
...
//Fim das configurações do código do semáforo
...
void MensagemSinalAmarelo(){
    
    char *text[] = {
        
        "   SINAL DE   ",
        "    ATENCAO   ",
        "  PREPARE-SE  ", // quando o LED amarelo         
    };
...
void MensagemSinalVermelho(){
    
    char *text[] = {
        
        " SINAL FECHADO ",
        "    AGUARDE    ",
        "               ", //quando o LED vermelho estiver         
    };
...
while(1){
    ...
    //SINAL VERDE PARA OS CARROS POR 8s
    SinalAberto(); 
    MensagemSinalVerde();   
    ...
    if(A_state){ //ALGUEM APERTOU O BOTAO - SAI DO SEMAFORO NORMAL
        ...
        SinalAtencao();         
        MensagemSinalAmarelo();        
        sleep_ms(5000);

        //SINAL VERMELHO PARA OS CARROS POR 10s        
        SinalFechado();
        MensagemSinalVermelho();        
        sleep_ms(10000);
        
    }else{ //NINGUÉM APERTOU O BOTAO - CONTINUA NO SEMAFORO NORMAL
        
        //SINAL AMARELO PARA OS CARROS POR 2s 
        SinalAtencao();
        MensagemSinalAmarelo();        
        sleep_ms(2000);
        
        //SINAL VERMELHO PARA OS CARROS POR "8s" (Alterado de 15s (atividade) para 8s para condizer com o código)
        SinalFechado();
        MensagemSinalVermelho();
        sleep_ms(8000); //Mantido o valor que consta na descrição da atividade 
        A_state = !A_state;
